...
class ListViewModelTest {
    ...
    fun `initial state should be Loading`() = runTest {
        ...
        Assert.assertTrue(viewModel.uiState.value is ListUiState.Loading)
    }
    ...
    fun `loadShows should return Success state with shows`() = runTest {
        ...
        viewModel.uiState.test {
            ...
            Assert.assertEquals(expectedShows, (state as ListUiState.Success).shows)
    ...
    fun `loadShows should return Error state on network failure`() = runTest {
        Mockito.`when`(repository.getShows(0)).thenThrow(RuntimeException("Network error"))

        viewModel = ListViewModel(repository, favouriteRepository)
        advanceUntilIdle()

        val state = viewModel.uiState.value
        Assert.assertTrue(state is ListUiState.Error)
    }
    ...
    fun `retry should reload data after error`() = runTest {
        ...
        Mockito.`when`(repository.getShows(0))
        ...
        Assert.assertTrue(viewModel.uiState.value is ListUiState.Error)

        viewModel.retry()
        advanceUntilIdle()

        val successState = viewModel.uiState.value
        Assert.assertTrue(successState is ListUiState.Success)
    }
    ...
    fun `empty search result should return Empty state`() = runTest {
        ...
        val state = viewModel.uiState.value
        Assert.assertTrue(state is ListUiState.Empty)
    }
}